...
for key in dict_data_trattamento.keys():
    ...
    try:
        ...
        dict_date_RM[key] = array_date_RM [array_date_RM > dict_data_trattamento[key] + delta]
        dict_date_RM[key].sort()
        D[key] = dict_date_RM[key][0]
    ...
    if file_RM_selected != []:
        ...
        for sequence in file_RM_selected:
            ...
            if (('_MR_' in sequence or '_mr_' in sequence)  and ('DWI' in sequence)):
...
export_excel = df.to_excel ('/home/leonardo/Scrivania/TESI/tabelle_MRI/estratte_da_me/DWI/export_info_RM_DWI_6_mesi.xlsx', index = None, header=True) #Don't forget to add '.xlsx' at the end of the path
